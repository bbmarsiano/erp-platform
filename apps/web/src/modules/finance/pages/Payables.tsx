...
import { usePayables, useRecordPayablePayment } from '../hooks/useFinance'

export default function Payables() {
  const payables = usePayables()
  const recordPayment = useRecordPayablePayment()
  const rows = useMemo(() => (payables.data ?? []) as Array<any>, [payables.data])
  ...
  return (
    <div style={{ padding: '28px 32px', maxWidth: 1400 }}>
      <PageHeader title="Задължения" subtitle="Проследяване на задължения към доставчици" />

      <div style={{ background: '#fff', border: '1px solid #e5e7eb', borderRadius: 12, overflow: 'hidden' }}>
        <table style={{ width: '100%', borderCollapse: 'collapse' }}>
          <thead>
            <tr style={{ borderBottom: '1px solid #e5e7eb', background: '#f9fafb' }}>
              <th style={financeTableThStyle}>Доставчик</th>
            ...
            {rows.length === 0 ? (
              ...
              rows.map((r) => {
                ...
                return (
                  ...
                  >
                    <td style={financeTableTdStyle}>{r.supplier?.name ?? '—'}</td>
          ...
          <div onClick={(e) => e.stopPropagation()} style={{ width: '100%', maxWidth: 420 }}>
            <Card>
              ...
              <p style={{ margin: '0 0 16px', fontSize: 13, color: '#6b7280' }}>
                {paymentTarget.supplier?.name} — фактура {paymentTarget.invoice?.number}
              </p>
