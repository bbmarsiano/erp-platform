...
export const useRegisters = () => useQuery({
...
export const useCreateRegister = () => {
    ...
    return useMutation({
        mutationFn: (data) => api.post('/api/pos/registers', data).then((r) => r.data.data),
...
export const useCreateSale = () => {
    const qc = useQueryClient();
    return useMutation({
        mutationFn: (data) => api.post('/api/pos/sales', data).then((r) => r.data.data),
        onSuccess: () => {
...
export const useRefundSale = () => {
    const qc = useQueryClient();
    return useMutation({
        mutationFn: (id) => api.post(`/api/pos/sales/${id}/refund`).then((r) => r.data.data),
        onSuccess: (_data, id) => {
            qc.invalidateQueries({ queryKey: ['pos', 'sales'] });
